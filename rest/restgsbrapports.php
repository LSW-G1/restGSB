...
class RestGSB extends Rest
{
	...
	private function test_updateMedecinTelephone($args)
	{
		$idMedecin = $args["idMedecin"];
		$telephone = $args["telephone"];

		$this->pdo->test_updateMedecinTelephone($idMedecin, $telephone);
	}

	private function utilisationMedicaments($args)
	{
		$this->data = $this->encoderReponse($this->pdo->getUtilisationMedicaments());
	}
	...
	public function process()
	{
		...
		*/
		...
		switch ($this->request['endpoint']) {
			...
			case 'sandbox_medicaments':
				if ($this->method == "GET") {
					$this->request['fonction'] = "utilisationMedicaments";
				} else {
					$this->response("", 400);
				}
				break;

			case 'test_updateMedecinTelephone':
				if ($this->method == "GET") {
					$this->request["fonction"] = "test_updateMedecinTelephone";
				} else {
					$this->response("Requête n'est pas en GET !", 400);
				}
			default:
		}
