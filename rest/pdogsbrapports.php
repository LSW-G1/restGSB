...
class PdoGsbRapports
{
	...
	public function getUtilisationMedicaments()
	{
		$req = "SELECT m.nomCommercial, COUNT(*) AS nbUtilisation FROM medicament m, offrir o WHERE m.id = o.idMedicament GROUP BY m.nomCommercial";
		$stm = self::$monPdo->prepare($req);
		$stm->execute();
	...
	public function test_updateMedecinTelephone($idMedecin, $telephone)
	{

		$req = "UPDATE medecin SET tel = :telephone WHERE id = :idMedecin;";
		$stm = self::$monPdo->prepare($req);
		$stm->execute([
			":telephone" => $telephone,
			":idMedecin" => $idMedecin
